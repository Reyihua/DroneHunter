...
from .encoder.builder import build_encoder
from .decoder.builder import build_decoder
import yaml
from .drop_path import DropPathAllocator
from .positional_encoding.builder import build_position_embedding
from timm.models.layers import trunc_normal_
#from .fine_moe import MLASimilarity
from .warm_moe import MLASimilarity
__all__ = ['SiamDTRCNN']
...
class SiamDTRCNN(TwoStageDetector):
    ...
    def __init__(self,
        ...
        with open('/root/code/SwinTrack/config/SwinTrack/Tiny/config.yaml', 'r') as file:
        ...
        self.z_pos = z_pos_enc
        self.x_pos = x_pos_enc

        with drop_path_allocator:
            self.encoder = build_encoder(config, drop_path_allocator,
                                transformer_dim, num_heads, mlp_ratio, qkv_bias, drop_rate, attn_drop_rate,
                                z_shape, x_shape)
            self.decoder = build_decoder(config, drop_path_allocator,
                                transformer_dim, num_heads, mlp_ratio, qkv_bias, drop_rate, attn_drop_rate,
                                z_shape, x_shape)
        ...
        self.mla = MLASimilarity()
        self.mla._init_weights()
        ...
        if return_loss:
            if epoch != None:
                ...
                return self.forward_train(
                    img_z, img_x, img_meta_z, img_meta_x, epoch, **kwargs)
            return self.forward_train(
    ...
    def forward_train(self,
        ...
        print(self.z_pos, self.x_pos)
        if self.z_pos is not None:
            z_pos = self.z_pos().unsqueeze(0)
        if self.x_pos is not None:
            x_pos = self.x_pos().unsqueeze(0)
        for i in template:
            print(i.size())
        for j in x:
            print(j.size())
        z_feat, x_feat = self.encoder(template[0].view(1, -1, 256), x[-1].view(1, -1, 256), z_pos, x_pos)
        decoder_feat = self.decoder(z_feat, x_feat, z_pos, x_pos)
        decoder_feat = self.out_norm(decoder_feat)
        decoder_feat = decoder_feat.view(1, 256, self.x_shape[0], self.x_shape[1])
        # Dual-Semantic Learning
        x_corr = next(self.rpn_similarity_learning(template, x))[0]
        x_corr[-1] = decoder_feat
        ...
        # Versatile learning
        bbox_feats_corr1 = self.rcnn_similarity_learning(
            bbox_feats_z, bbox_feats_x)

        #self.mla.set_epoch(self._epoch)
        """if self._epoch < 18:
            self.mla.requires_grad_(True)
        else:
            self.mla.eval()
            self.mla.requires_grad_(False)
        bbox_feats_corr = self.mla(bbox_feats_z, bbox_feats_corr1)"""
        bbox_feats_corr = bbox_feats_corr1
