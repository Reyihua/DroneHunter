...
class SiamDTRCNN(TwoStageDetector):
    ...
    def forward_train(self,
        ...
        for i in x_corr:
            ...
            print(i.size())
        # Concat x and x_corr
        x_concat = [x[i] + x_corr[i] for i in range(len(x))]#[x[i] + x_corr[i] for i in range(len(x))]
        ...
        bbox_feats_corr = self.mla(bbox_feats_z, bbox_feats_x)
        balance_loss = 0.0001 * self.mla.compute_balance_loss()
        balance_loss = {'balance_loss': balance_loss}
        losses.update(balance_loss)
