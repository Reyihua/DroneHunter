...
class antiUAVV(SeqDataset):

    r"""`antiUAVV`_ Datasets.
    ...
    def __init__(self, root_dir=None, subset='test', download=True):
        ...
        self.name = 'antiUAVV_{}'.format(subset)

        # initialize the dataset
        super(antiUAVV, self).__init__(
            name=self.name,
    ...
    def _construct_seq_dict(self, root_dir, subset):
        ...
        for s, seq_name in enumerate(seq_names):
            ...
            with open(anno_files[s], 'r') as f:
                ...
                # 假设anno中的每个元素应为包含四个浮点数的列表
                for i, item in enumerate(anno):
                    if len(item) != 4:
                        print(f"Index {i} has incorrect length: {seq_name}")
